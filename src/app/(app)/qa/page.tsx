    ...
    <div className="h-full flex flex-col">
      ...
      <main className="flex-1 flex flex-col overflow-hidden px-4 sm:px-6 md:px-8 pb-4">
        <div className="w-full flex flex-col flex-1">
          ...
          <div className="flex-1 flex flex-col gap-6 overflow-y-auto rounded-lg border bg-card p-4 mx-auto w-full max-w-4xl">
            {messages.length === 0 && (
              <div className="flex flex-1 flex-col items-center justify-center text-center">
                ...
                <p className="text-muted-foreground max-w-md mt-2">
                  Ask questions across your sources to surface insights, decisions, and updates — like
                  <span className="italic"> “What were the biggest wins last quarter?”</span>
                </p>
              </div>
            )}
            ...
            {messages.map((m) => (
              <div key={m.id} className={cn('flex items-start gap-4', m.role === 'user' && 'justify-end')}>
                ...
                <div
                  className={cn(
                    'rounded-lg px-4 py-3 flex flex-col gap-2',
                    m.role === 'user'
                      ? 'bg-primary text-primary-foreground max-w-xl'
                      : 'bg-muted w-full max-w-4xl'
                  )}
                >
